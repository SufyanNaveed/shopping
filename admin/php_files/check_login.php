...
if(isset($_POST['login'])){
    ...
    if(!isset($_POST['name']) || empty($_POST['name'])){
        ...
        $db->select('admin','admin_id,admin_name',null,"username = '$username' AND password = '$password'",null,0); 
        $result = $db->getResult();
...
if(isset($_POST['changePass'])){
    if(!isset($_POST['old_pass']) || empty($_POST['old_pass'])){
        ...
        // print_r($_POST);exit;
        $db->update('admin',array('password'=>$new, 'username' => $_POST['username'],'admin_name' => $_POST['username']),"admin_id = '{$_POST['admin_id']}'");
